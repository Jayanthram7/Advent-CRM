...
// GET /api/leads/analytics (must be before /:id)
router.get('/analytics', async (req, res) => {
  try {
    ...
    const [total, converted, open, followUp, installation] = await Promise.all([
      ...
      Lead.countDocuments({ ...baseQuery, isConverted: false, labels: 'Open' }),
      Lead.countDocuments({ ...baseQuery, isConverted: false, labels: 'Follow Up' }),
      Lead.countDocuments({ ...baseQuery, installationDate: { $exists: true, $ne: null } }),
    ]);
    ...
    const labelAgg = await Lead.aggregate([
    ...
    const topFollowedAgg = await Lead.aggregate([
      { $match: baseQuery },
      { $project: {
    ...
    const topFollowed = topFollowedAgg.map(d => ({
      id: d._id,
      name: d.name ? d.name.trim() : (d.company || d.phone || 'Unnamed Lead'),
    ...
    const followsAgg = await Lead.aggregate([
      { $match: baseQuery },
    ...
    const installationsInRange = await Lead.countDocuments({
      ...baseQuery,
    ...
    res.status(500).json({ message: 'Error fetching leads analytics' });
  }
});
