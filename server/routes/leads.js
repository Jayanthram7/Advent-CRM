...
router.post('/:id/date', async (req, res) => {
  try {
    ...
    const update = {};
    if (callbackDate) update.callbackDate = new Date(callbackDate);
    if (followUpDate) update.followUpDate = new Date(followUpDate);
    if (installationDate) update.installationDate = new Date(installationDate);
    ...
    await Activity.create({
      lead: lead._id,
      type: 'DateUpdate',
      content: `${dateType} date set to ${formatDate(callbackDate || followUpDate || installationDate)} by ${req.user.name}`,
      performedBy: req.user._id
    });
